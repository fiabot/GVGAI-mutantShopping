...
public class LevelChain {
    ...
    HashMap<String, ArrayList<Integer>> countMap; // counts, used to calculate prob 
    ...
    public LevelChain(char[] tiles, int seed){
        this.seed = seed; 
    ...
    public void train(String[] level_files){
        ...
        for(String key : countMap.keySet()){
            ArrayList<Integer> counts = countMap.get(key); 
            float sum = 0; 
            for (int i: counts){
                sum += i; 
            }
            ...
            if (sum == 0){
                ...
                for(int i = 0; i < counts.size(); i ++){
                    probMap.get(key)[i] = counts.get(i) / sum;                 
                }
            }
        }    
    ...
    public char[][] generalizedLevel(String game, String levelFile){
        char[][] level = getLevelArray(levelFile); 
        return new char[0][0]; 
    }
    ...
    public void updateCounts(char[][] levelArray){
        ...
        for(int y = 0; y < levelArray.length; y++){
			for(int x = 0; x <levelArray[0].length; x++){
                ...
                ArrayList<Integer> currentCounts = countMap.get(getContext(x, y, levelArray)); 
                currentCounts.set(tileToIndex.get(thisChar), currentCounts.get(tileToIndex.get(thisChar) + 1)); 
